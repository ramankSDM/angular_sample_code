...
export class ListCelebritiesComponent implements OnInit {
  ...
  /*
  Name: getAll
  Description: Get list of all Celebrities based on tag id.
  */
  getAll() {
    this.spinner.show();
    let path = `celebrities/get-all-celebrity-by-tagId`;
    let options = {
      id: this.id,
  ...
  */
  ...
  /*
  Name: updateRank
  Description: Update the Rank
  */
  updateRank(key,rank) {
    this.spinner.show();
    let path = `usertags/updateRank`;
    let json = {
      id: key,
      rank: rank
    };
