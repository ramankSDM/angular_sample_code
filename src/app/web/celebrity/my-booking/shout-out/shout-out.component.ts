...
export class ShoutOutComponent implements OnInit {
  ...
  buttonVideoEnabled:boolean = true
  buttonImageEnabled:boolean = true
  buttonVideoMessageShow:boolean = false
  buttonImageMessageShow:boolean = false
  msaapPlaylist: Track[] = [
    {
  ...
  */
  ...
  // publish messages public and private
    ...
    if (event && event.target && event.target.files && event.target.files.length > 0) {
      if (event.target.files[0].size > this.videoFile) {
        ...
        this.commonService.post(apiPath, formData).subscribe(result => {
          ...
          this.spinner.hide();
          this.buttonVideoEnabled=false;
          this.buttonVideoMessageShow=false
        }, err => {
          this.spinner.hide();
          this.commonService.handleError(err);
        });
      ...
      reader.onload = (e: any) => {
        ...
        image.onload = rs => {
          ...
          this.commonService.post(apiPath, { "image": imgBase64Path }).subscribe(result => {
            ...
            this.spinner.hide();
            this.buttonImageEnabled=false
            this.buttonImageMessageShow=false
          }, err => {
            this.spinner.hide();
            this.commonService.handleError(err);
          });
  ...
  */
  complete() {
    if(this.buttonImageEnabled){
      this.buttonImageMessageShow=true
      return
    }
    if(this.buttonVideoEnabled){
      this.buttonVideoMessageShow=true
      return
    }
