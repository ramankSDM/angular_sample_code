...
export class WebinarComponent implements OnInit {
  ...
  */
  getAll() {
    ...
    console.log('options webinar',options)
    this.commonService.queryParams(path, options).subscribe(response => {
      this.dataList = response.data.data;
      console.log('this.dataList webinar',this.dataList)
  ...
  */
  cancelRequest(itemValue) {
    this.spinner.show();
    let path = `booking/updateStatus`;
    let json = {
      requestId: itemValue._id,
      status: 4
    };
    this.commonService.put(path, json).subscribe(response => {
  ...
  */
  launchMeeting(item){
    ...
    let options = {
      meetingId: item.webinar.webinar_id     
    };
    this.commonService.queryParams(path, options).subscribe(response => {
