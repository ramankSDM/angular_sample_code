...
export class TimelineComponent implements OnInit {
  ...
  */
  ...
  // }
  // returnState(e, ind) {
  //   e.target.closest('.default-carousel').previousSibling.previousSibling.style.display = 'block';
  //   // this.showCarouselBool = false;
  //   var elem = document.getElementsByClassName('temp');
  //   while (elem[0]) { elem[0].parentNode.removeChild(elem[0]); }
  //   this[ind] = false;
  // }
  ...
  */
  timelineDetail(data) {
    this.router.navigate(['/fans/timeline-detail/' + data._id])
  }
  ...
  */
  likeUnlike(id, status) {
    ...
    this.commonService.post(path, obj).subscribe(response => {
      ...
      this.getBookings(1, 5);
      this.gotTalentBookings(2, 5);
      this.spinner.hide();
    }, err => {
      this.spinner.hide();
      this.commonService.handleError(err);
